...
public class DataTable{
	...
	public DataTable(ResultSet rs) throws SQLException{
		System.out.println("DataTable: DataTable(ResultSet)");
		ResultSetMetaData rsmd=rs.getMetaData();
		for(int i=1; i<=rsmd.getColumnCount(); i++){
			System.out.println("DataTable: DataTable(ResultSet): creating column ["+i+"] from ResultSet");
			columns.add(new DataColumn(rsmd,i));
		}
		while (rs.next()){
			System.out.println("DataTable: DataTable(ResultSet): processign result set row");
